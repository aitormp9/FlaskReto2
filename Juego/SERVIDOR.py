...
def generar_muros():
    ...
    for z in zonas:
        for _ in range(4):
            muros.append(
                f"{random.randint(z[0], z[2])},{random.randint(z[1], z[3])},{random.randint(20, 100)},{random.randint(20, 100)}")
    return "|".join(muros)
...
class ServerLogic:
    def __init__(self):
        ...
        self.jugador = None  # ID del portador
        self.en_casa = False
        self.t_entrega = 0
        self.jugadores = {}  # {id: [x, y]}
    ...
    def estadobandera(self):
        ...
        for id_j, p in list(self.jugadores.items()):
            ...
            # 1. ROBO: Si alguien tiene la bandera y lo tocas
            if self.jugador is not None and self.jugador != id_j and self.jugador != "CASA":
                if rj.colliderect(rb):
                    id_pillado = self.jugador
                    self.jugadores[id_pillado] = pos_inicios[id_pillado % 4][:]
                    self.jugador = None
...
def manejar_cliente(conn, id_j):
    ...
    clientes[id_j] = conn
    try:
        # Enviar mapa y posición inicial
        ...
        while True:
            ...
            # Paquete: Jugadores | Bandera
            p_jugs = "|".join([f"{i}:{p[0]},{p[1]}" for i, p in game.jugadores.items()])
