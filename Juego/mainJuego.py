...
def contador():
    # Definimos las posiciones para cada uno de los 4 jugadores
    posiciones = [
    ...
    for i in range(len(rondas)):
        # Renderizamos el texto de cada jugador
        texto = f"{rondas[i]}"
        texto_surface = fuente_contador.render(texto, True, (255,255,255))

        # Ajustamos el rect para que si es el lado derecho, se alinee a la derecha
        rect = texto_surface.get_rect()
        if posiciones[i][0] > 640:  # Si está a la derecha
...
def estadobandera():
    ...
    for jugador in jugadores:
        ...
        if bandera.jugador==None and jugador.getrect().colliderect(bandera.getrect()):
            ...
            #print(webFlask.jugador_bandera)
            print(jugador)
